...
function TaskRow({ task, completed, pending, finishing, onToggle }: { task: TodayTask; completed: boolean; pending: boolean; finishing: boolean; onToggle: () => void }) {
  ...
  return <article className={`focus-task-row group ${completed ? "focus-task-completed" : ""} ${finishing ? "focus-task-finishing" : ""}`}><button type="button" onClick={onToggle} disabled={pending} className={completed ? "focus-reopen-button" : `task-check ${finishing ? "completion-check-bloom" : ""}`} aria-label={completed ? `Reopen ${task.title}` : `Complete ${task.title}`}>{completed ? <RotateCcw size={13} /> : <span className="task-check-surface">{finishing ? <Check size={13} strokeWidth={3} /> : null}</span>}</button><div className="min-w-0 flex-1"><p className={`text-sm font-medium leading-5 ${completed || finishing ? "text-muted-foreground line-through decoration-[var(--orange)]/45" : ""}`}>{task.title}</p><p className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground"><span>{date}</span>{task.estimatedMinutes ? <><span>·</span><Clock3 size={11} /> {task.estimatedMinutes} min</> : null}<span>·</span><span>{finishing ? "Completed" : statusLabels[task.status]}</span></p></div></article>;
}
