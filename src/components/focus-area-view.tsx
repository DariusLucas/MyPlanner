...
import { Check, Clock3, Flag, Pencil, Plus, Settings2, Trash2, X } from "lucide-react";
...
import { TaskCompletionButton } from "@/src/components/interaction-primitives";
import { COMPLETION_FEEDBACK_MS } from "@/src/lib/interaction";
...
export function FocusAreaView({ data }: { data: FocusAreaData }) {
  ...
  async function toggle(task: TodayTask) {
    ...
    setPendingTaskIds((current) => { const next = new Set(current); next.delete(task.id); return next; });
    if (completing) window.setTimeout(() => setFinishingIds((current) => { const next = new Set(current); next.delete(task.id); return next; }), COMPLETION_FEEDBACK_MS);
    else setFinishingIds((current) => { const next = new Set(current); next.delete(task.id); return next; });
  }
  ...
  return <div className="focus-area-shell mx-auto w-full max-w-[1500px] space-y-6 sm:space-y-7">
    <header className="flex flex-wrap items-end justify-between gap-4"><h1 className="text-3xl font-semibold tracking-[-0.055em] sm:text-[2.6rem]">{title}</h1><div className="flex items-center gap-2"><button onClick={() => setEditingCategory(true)} className="thought-quiet-button"><Settings2 size={15} /> Edit category</button><button onClick={() => setComposer(true)} className="premium-primary-button"><Plus size={16} /> Add task</button></div></header>
    ...
    {composer && <TaskComposer weekStart={weekStart} days={weekDays} defaultCategory={data.category.id} lockCategory pending={pending === "create-task"} onSubmit={addFocusTask} onClose={() => setComposer(false)} />}
    <CategoryDialog open={editingCategory} category={data.category} onClose={() => setEditingCategory(false)} onDeleted={() => router.push("/")} />
...
function TaskRow({ task, completed, pending, finishing, onToggle }: { task: TodayTask; completed: boolean; pending: boolean; finishing: boolean; onToggle: () => void }) {
  ...
  return <article className={`focus-task-row group ${completed ? "focus-task-completed" : ""} ${finishing ? "focus-task-finishing" : ""}`}><TaskCompletionButton title={task.title} completed={completed} pending={pending} animating={finishing} onClick={onToggle} /><div className="min-w-0 flex-1"><p className={`text-sm font-medium leading-5 ${completed || finishing ? "text-muted-foreground line-through decoration-[var(--orange)]/45" : ""}`}>{task.title}</p><p className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground"><span>{date}</span>{task.estimatedMinutes ? <><span>·</span><Clock3 size={11} /> {task.estimatedMinutes} min</> : null}<span>·</span><span>{finishing ? "Completed" : statusLabels[task.status]}</span></p></div></article>;
}
...
function MilestonesPanel({ category, milestones, pending, run }: { category: PlannerCategory; milestones: FocusAreaData["milestones"]; pending: string | null; run: Runner }) {
  ...
  return <section className="glass-panel overflow-visible rounded-[28px]"><div className="focus-card-heading milestone-heading"><div className="flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-accent text-accent-foreground"><Flag size={18} /></span><div><p className="dashboard-eyebrow">{category.name} milestones</p><p className="mt-1 text-sm text-muted-foreground">Outcomes worth remembering, separate from task progress.</p></div></div><button onClick={() => setAdding(true)} className="focus-add-button"><Plus size={14} /> Add milestone</button></div><AnimatedPresence show={adding} className="milestone-form-presence"><MilestoneForm category={category.id} submitLabel="Add milestone" pending={pending === "milestone-new"} onCancel={() => setAdding(false)} action={(form) => run("milestone-new", createMilestone, form, () => setAdding(false))} /></AnimatedPresence><div className="milestone-list border-t border-border p-3 sm:p-4">{all.length ? <div className="grid gap-2 sm:grid-cols-2">{all.map((milestone) => editing === milestone.id ? <div key={milestone.id} className="entity-editor-enter"><MilestoneForm category={category.id} milestone={milestone} submitLabel="Save" pending={pending === `milestone-${milestone.id}`} onCancel={() => setEditing(null)} action={(form) => run(`milestone-${milestone.id}`, updateMilestone, form, () => setEditing(null))} /></div> : <MilestoneRow key={milestone.id} category={category.id} milestone={milestone} pending={pending === `milestone-${milestone.id}`} confirming={confirmDelete === milestone.id} onToggle={(form) => run(`milestone-${milestone.id}`, toggleMilestone, form)} onEdit={() => setEditing(milestone.id)} onConfirmDelete={() => setConfirmDelete(milestone.id)} onCancelDelete={() => setConfirmDelete(null)} onDelete={(form) => run(`milestone-${milestone.id}`, deleteMilestone, form, () => setConfirmDelete(null))} />)}</div> : <div className="entity-empty-state py-8 text-center"><p className="text-sm font-semibold">No milestones yet.</p><p className="mt-1 text-xs text-muted-foreground">Add a meaningful outcome when you have one.</p></div>}</div></section>;
}
