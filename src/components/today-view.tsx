...
const inputClass = "w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none transition placeholder:text-muted-foreground/70 focus:border-ring focus:ring-2 focus:ring-ring/15";
const quietButton = "inline-flex h-9 items-center justify-center gap-2 rounded-lg border border-border bg-card px-3 text-sm text-muted-foreground transition hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50";
    ...
    <div className="space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-0.5 text-2xl font-semibold tracking-tight sm:text-3xl">{isToday(selectedDate) ? "Today" : format(selectedDate, "EEEE")}</h1>
        </div>
        <div className="flex items-center gap-1.5">
          <Link href={`/today?date=${previousDate}`} className={quietButton} aria-label="Previous day"><ArrowLeft size={16} /></Link>
          <Link href={`/today?date=${data.date}`} className={`${quietButton} ${isToday(selectedDate) ? "bg-foreground text-background hover:bg-foreground hover:text-background" : ""}`}><CalendarDays size={15} /> Today</Link>
      ...
      <section className="rounded-xl border border-border bg-card p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3"><div className="grid size-9 shrink-0 place-items-center rounded-lg bg-muted text-sm font-semibold">{data.completed}/{data.total}</div><div><h2 className="font-semibold">Today’s checklist</h2><p className="text-xs text-muted-foreground">{data.total === 0 ? "Start with one useful action." : `${data.completed} complete · ${data.total - data.completed} left`}</p></div></div>
          <button type="button" onClick={() => setShowCreate((value) => !value)} className="inline-flex h-9 items-center gap-2 rounded-lg bg-foreground px-3 text-sm font-medium text-background transition hover:opacity-90"><Plus size={16} /> Add task</button>
        </div>
        <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-foreground transition-[width]" style={{ width: `${progress}%` }} /></div>
      ...
      <details className="group rounded-xl border border-border bg-card" open={hasFocus}>
        <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 text-sm font-medium"><span>Daily focus <span className="ml-1 text-xs font-normal text-muted-foreground">(optional)</span></span><ChevronDown size={16} className="text-muted-foreground transition-transform group-open:rotate-180" /></summary>
...
function CategorySection({ category, tasks, completed, total, editingId, setEditingId, pending, runAction }: { category: TaskCategory; tasks: TodayTask[]; completed: number; total: number; editingId: number | null; setEditingId: (id: number | null) => void; pending: string | null; runAction: (key: string, action: (formData: FormData) => Promise<ActionResult>, formData: FormData, onSuccess?: () => void) => Promise<void> }) {
  return <section>
    <div className="mb-2 flex items-center gap-2"><h3 className="text-xs font-semibold uppercase tracking-[0.11em] text-muted-foreground">{categoryLabels[category]}</h3>{total > 0 && <span className="text-xs text-muted-foreground">{completed}/{total}</span>}</div>
    {tasks.length === 0 ? <p className="rounded-lg bg-muted/60 px-3 py-3 text-sm text-muted-foreground">No tasks yet. Add the next concrete action.</p> : <div className="divide-y divide-border overflow-hidden rounded-lg border border-border">{tasks.map((task, index) => <TaskItem key={task.id} task={task} index={index} last={tasks.length - 1} editing={editingId === task.id} setEditingId={setEditingId} pending={pending} runAction={runAction} />)}</div>}
  </section>;
}
...
function ReorderButton({ task, direction, pending, runAction, children }: { task: TodayTask; direction: "up" | "down"; pending: boolean; runAction: (key: string, action: (formData: FormData) => Promise<ActionResult>, formData: FormData, onSuccess?: () => void) => Promise<void>; children: React.ReactNode }) {
  return <form action={(formData) => runAction(`${direction}-${task.id}`, reorderTask, formData)}><input type="hidden" name="id" value={task.id} /><input type="hidden" name="date" value={task.date} /><input type="hidden" name="category" value={task.category} /><input type="hidden" name="direction" value={direction} /><button type="submit" disabled={pending} className="rounded-md border border-border bg-card p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label={`Move ${direction}`}>{children}</button></form>;
}
...
function TaskForm({ date, task, pending, onSubmit, onCancel, submitLabel }: { date: string; task?: TodayTask; pending: boolean; onSubmit: (formData: FormData) => Promise<void>; onCancel?: () => void; submitLabel: string }) {
  return <form className="mt-4 rounded-lg bg-muted/50 p-3" action={onSubmit}>
    ...
    <div className="mt-3 flex justify-end gap-2"><button type="submit" disabled={pending} className="inline-flex h-9 items-center rounded-lg bg-foreground px-3 text-sm font-medium text-background transition hover:opacity-90 disabled:opacity-50">{pending ? "Saving…" : submitLabel}</button>{onCancel && <button type="button" onClick={onCancel} className={quietButton}>Cancel</button>}</div>
  </form>;
}
