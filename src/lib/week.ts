...
export function getWeekData(weekStart: string): WeekData {
  ...
  const completed = db.select().from(tasks).where(and(gte(tasks.date, weekStart), lte(tasks.date, weekEnd), eq(tasks.status, "completed"))).orderBy(asc(tasks.date), asc(tasks.position), asc(tasks.id)).all();
  ...
  return {
    ...
    weekEnd,
    days,
    anytime: active.filter((task) => task.anytimeWeekStart === weekStart),
